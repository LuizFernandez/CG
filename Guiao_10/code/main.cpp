...
GLuint vertexCount, normalCount;
GLuint vertices[2];
...
void prepareCilinder(float height, float radius, int sides) {
	...
	for (int i = 0; i < sides; ++i) {
		...
		v[vertex*3 + 2] = radius * cos( (i+1) * delta);
        normal++;
        n[normal*3 + 0] = sin((i+1) * delta);
        n[normal*3 + 1] = 0.0f;
        n[normal*3 + 2] = cos((i+1) * delta);
		...
		v[vertex*3 + 2] = radius * cos( i * delta);
        normal++;
        n[normal*3 + 0] = sin(i * delta);
        n[normal*3 + 1] = 0.0f;
        n[normal*3 + 2] = cos(i * delta);
