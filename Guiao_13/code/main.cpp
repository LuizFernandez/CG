...
void prepareCylinder(float altura, float raio, int lados) {
    ...
    float var_lado = 1.0f / lados;

    float raio_t = 0.625 - 0.4375;
	...
	for (int i = 0; i < lados; ++i) {
		...
		n[vertex*3 + 1] = 1.0f;
		n[vertex*3 + 2] = 0;
        t.push_back(0.4375f + raio_t * sin(i * delta));
        t.push_back(0.1875f + raio_t * cos(i * delta));
		...
		n[vertex*3 + 1] = 1.0f;
		n[vertex*3 + 2] = 0;
        t.push_back(0.4375f + raio_t * sin((i+1) * delta));
        t.push_back(0.1875f + raio_t * cos((i+1) * delta));
		...
		n[vertex*3 + 1] = -1.0f;
		n[vertex*3 + 2] = 0.0f;
        t.push_back(0.8125f + raio_t * sin((i+1) * delta));
        t.push_back(0.1875f + raio_t * cos((i+1) * delta));
		v[vertex*3 + 0] = raio * sin( (i+1) * delta);
		v[vertex*3 + 1] = -altura/2.0f;
		v[vertex*3 + 2] = raio * cos( (i+1) * delta);
		...
		n[vertex*3 + 1] = -1.0f;
		n[vertex*3 + 2] = 0.0f;
        t.push_back(0.8125f + raio_t * sin(i * delta));
        t.push_back(0.1875f + raio_t * cos(i * delta));
		v[vertex*3 + 0] = raio * sin( i * delta);
		v[vertex*3 + 1] = -altura/2.0f;
		v[vertex*3 + 2] = raio * cos( i * delta);
...
int loadTexture(std::string s) {
    ...
    // for each image ...
	ilGenImages(1,&t); // unsigned int t[...]
	ilBindImage(t);
	ilLoadImage((ILstring)s.c_str()); // char* filename
