...
void cylinder(float radius, float height, int sides) {
	...
	// Criar um vetor com os dados dos pontos
	int i;
	float step;
	...
	// Copiar o Vector para a memoria Grafica
	glBindBuffer(GL_ARRAY_BUFFER, vertices);
	glBufferData(
		GL_ARRAY_BUFFER, // Tipo do Buffer, so e relevante na altura do desenho
		sizeof(float) * p.size(), // Tamanho do vetor em bytes
		p.data(), // Os dados do array associado ao vetor
		GL_STATIC_DRAW); // Indicativo de utilizaçao (Estatico e para desenho)
...
void renderScene(void) {
	...
	if((time - timebase) > 1000){
		...
		timebase = time;
		frames = 0;
		sprintf(title_fps, "%0.3f", fps);
		glutSetWindowTitle(title_fps);
	}
