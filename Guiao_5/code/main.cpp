...
void renderScene(void) {
	...
	//Torus no Centro do Mapa
	glColor3f(1.0f, 0.5f, 0.0f);
	glTranslatef(0,1,0);
	glRotatef(90, 1, 0, 0);
	glutSolidTorus(1,2,15,20);
	glRotatef(-90, 1, 0, 0);
	glTranslatef(0,-1,0);

	//Primeiro Circulo de Teapot a Rodar à volta do Torus
	glColor3f(1.0f, 0.0f, 0.0f);
	for(int i = 0; i < 8; i++){
		glPushMatrix();
		glRotatef(45 * i, 0, 1, 0);
		glTranslatef(rc, 1.5, 0);
		glutSolidTeapot(2);
		glPopMatrix();
	}

	//Segundo Circulo de Teapot a Rodar à volta do Torus
	glColor3f(0.0f, 0.5f, 0.5f);
	for(int i = 0; i < 16; i++){
		glPushMatrix();
		glRotatef(22.5 * i, 0, 1, 0);
		glTranslatef(ri, 1.5, 0);
		glutSolidTeapot(2);
		glPopMatrix();
	}

	//Arvores à volta dos Circulos
	for(int i = 0; i < 300; i++) {
		glPushMatrix();
